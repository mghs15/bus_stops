...
　　　　if(sche_day == ""){sche_day <- "<span style='color: #FFA500;'>運行情報注意</span>"} #本当か？
  }
...
stop_times$calendar_pattern[is.na(stop_times$calendar_pattern)] <- "<span style='color: #FFA500;'>運行情報注意</span>" 
#定期運行のみ（calendar.txtに存在するもの）が対象となる。
...
for(i in 1:N){
  ...
  .times_head$calendar_pattern[is.na(.times_head$calendar_pattern)] <- "<span style='color: #FFA500;'>運行情報注意</span>"
...
stops.df.n@data <- stops.df.n@data %>% mutate( bikou=rep(paste(Sys.Date(), "時点の情報で作成"), nrow(stops.df.n@data)) )
...
output_tile_name <- paste(path, "stops_", agency_id, "_", as.character(tag), ".geojson", sep="") 
writeOGR(stops.df.n, output_tile_name , layer="layer", driver="GeoJSON", encoding="SJIS")

#output agency information
agency_info <- paste(agency_name, agency_id, paste("stops_", agency_id, "_", as.character(tag), ".geojson", sep=""), sep="\t")
